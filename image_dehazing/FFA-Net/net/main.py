...
T=opt.steps	
def lr_schedule_cosdecay(t,T,init_lr=0.0001): #opt.lr):
  a=1+math.cos(t*math.pi/T)
...
def train(net,loader_train,loader_test,optim,criterion):
	...
	for step in range(start_step+1,opt.steps+1):
		...
		print(f'\rtrain loss : {loss.item():.5f}| step :{step}/{opt.steps}|lr :{lr :.15f} |time_used :{(time.time()-start_time)/60 :.1f}',end='',flush=True)
    # print(f'\nstep :{step} |max_ssim:{max_ssim:.4f}| max_psnr:{max_psnr:.4f}')
