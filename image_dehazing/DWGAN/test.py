...
parser.add_argument('--test_dir', type=str, default='./input_set/test1/')
parser.add_argument('--output_dir', type=str, default='./outputs/test2/')
...
net.load_state_dict(torch.load('./weights/dehaze.pkl', map_location=torch.device('cpu')))
# --- Test --- #
with torch.no_grad():
    net.eval()
    ...
    for batch_idx, (hazy_up,hazy_down,name) in enumerate(test_loader):
        ...
        imwrite(frame_out, output_dir + '/' + str(name[0])  , range=(0, 1))
